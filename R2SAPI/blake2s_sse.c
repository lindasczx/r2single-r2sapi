...
#define STORE(p,r) _mm_storeu_si128((__m128i *)(p), r)
...
int blake2s_compress_sse( blake2s_state *S, const byte block[BLAKE2S_BLOCKBYTES] )
{
	...
	const __m128i crotr8 = _mm_set_epi8( 12, 15, 14, 13, 8, 11, 10, 9, 4, 7, 6, 5, 0, 3, 2, 1 );
	const __m128i crotr16 = _mm_set_epi8( 13, 12, 15, 14, 9, 8, 11, 10, 5, 4, 7, 6, 1, 0, 3, 2 );
#endif

	const uint32_t  *m = ( uint32_t * )block;

	row[0] = ff0 = LOAD( &S->h[0] );
	row[1] = ff1 = LOAD( &S->h[4] );

	row[2] = _mm_setr_epi32( 0x6A09E667, 0xBB67AE85, 0x3C6EF372, 0xA54FF53A );
	row[3] = _mm_xor_si128( _mm_setr_epi32( 0x510E527F, 0x9B05688C, 0x1F83D9AB, 0x5BE0CD19 ), LOAD( &S->t[0] ) );
	SSE_ROUND( m, row, 0 );
	SSE_ROUND( m, row, 1 );
	SSE_ROUND( m, row, 2 );
	SSE_ROUND( m, row, 3 );
